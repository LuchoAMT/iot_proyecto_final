...
const int PORT = 1883; // Puerto para conexión segura
const char* TOPIC = "panconqueso/sensores/temperaturas";
...
void loop() {
  //if (wifiManager.isConnected() && mqttClient.isConnected()) {
    ...
    // Publicar los datos de los sensores en el broker MQTT
    String payload = String("{\"tempDHT\": ") + tempDHT + ", \"tempMLX\": " + tempMLX + "}";
    mqttClient.publishMessage(payload.c_str());
...
//se logró mandar la información a aws.
